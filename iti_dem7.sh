#!/bin/bash
# ==============================================================================
# Master Script - V7 (Фикс DHCP-сервера для Способа 2, задержки запуска)
# ==============================================================================
...
echo -e "${CYAN}Лечение DNS и удаление CD-ROM из источников...${NC}"
...
apply_netplan() {
    echo -e "${CYAN}Применение Netplan...${NC}"
...
setup_hqrtr() {
    ...
    echo -e "\n${YELLOW}=== Выбор топологии локальной сети HQ ===${NC}"
    echo "1) Один кабель (Транк). SRV и CLI висят на одном интерфейсе через VLAN."
    echo "2) Разные кабели. SRV и CLI подключены в разные сетевые адаптеры роутера."
    read -p "Твой выбор (1 или 2): " TOPO_CHOICE
    ...
    if [ "$TOPO_CHOICE" == "1" ]; then
        read -p "Интерфейс ВНУТРЬ (Один кабель для VLANs): " INT_LAN
        cat <<EOF > /etc/netplan/00-config.yaml
network:
  version: 2
  ethernets:
...
EOF
        ...
        read -p "Интерфейс к HQ-SRV: " INT_SRV
        read -p "Интерфейс к HQ-CLI: " INT_CLI
        cat <<EOF > /etc/netplan/00-config.yaml
network:
  version: 2
  ethernets:
...
EOF
    ...
    # ИСПРАВЛЕНИЕ DHCP: Жестко задаем интерфейс и перезапускаем службу
    echo -e "${CYAN}Настройка DHCP-сервера на интерфейсе $DHCP_IFACE...${NC}"
...
EOF
    ...
    # Даем интерфейсу время подняться, прежде чем стартовать DHCP
    sleep 2
    systemctl restart isc-dhcp-server || {
        echo -e "${RED}ВНИМАНИЕ: DHCP сервер не запустился. Проверьте лог: journalctl -xeu isc-dhcp-server${NC}"
    }
...
setup_hqsrv() {
    ...
    echo -e "\n${YELLOW}=== Выбор подключения к роутеру ===${NC}"
    echo "1) Подключен через Транк (Один кабель). Нужно настроить VLAN 10 на самом сервере."
    echo "2) Прямой кабель. Роутер уже дает чистую сеть, VLAN не нужен."
    read -p "Твой выбор (1 или 2): " TOPO_CHOICE
    
    show_interfaces
    read -p "Основной интерфейс (кабель к сети/роутеру): " INT_LAN

    echo -e "${CYAN}Установка пакетов...${NC}"
    apt-get install -y -q mdadm nfs-kernel-server bind9 bind9utils chrony apache2 mariadb-server php libapache2-mod-php php-mysql php-xml php-curl php-zip php-gd php-mbstring php-intl unzip wget

    echo -e "${CYAN}Пред-загрузка Moodle из интернета...${NC}"
    wget -qO /tmp/moodle.zip https://packaging.moodle.org/stable404/moodle-latest-404.zip || true
    unzip -q -o /tmp/moodle.zip -d /var/www/html/ || true
    chown -R www-data:www-data /var/www/html/moodle || true
...
EOF
    ...
    echo -e "${GREEN}[УСПЕХ] HQ-SRV настроен! (Moodle распакован в /var/www/html/moodle)${NC}"
}
...
setup_brsrv() {
    ...
    echo -e "${CYAN}Загрузка образов Docker из интернета...${NC}"
    systemctl enable docker --now
    docker pull mediawiki:latest || true
...
setup_hqcli() {
    ...
    echo -e "\n${YELLOW}=== Выбор подключения к роутеру ===${NC}"
    echo "1) Подключен через Транк (Один кабель). Нужно настроить VLAN 20 на клиенте."
    echo "2) Прямой кабель. Роутер уже дает чистую сеть, VLAN не нужен."
    read -p "Твой выбор (1 или 2): " TOPO_CHOICE
    ...
    echo -e "${CYAN}Установка пакетов...${NC}"
    export DEBIAN_FRONTEND=noninteractive
    wget -qO - https://repo.yandex.ru/yandex-browser/YANDEX-BROWSER-KEY.GPG | gpg --dearmor -o /usr/share/keyrings/yandex-browser.gpg || true
...
clear
echo -e "${YELLOW}=================================================${NC}"
echo -e "${GREEN} ЭКЗАМЕН - ИНТЕЛЛЕКТУАЛЬНЫЙ ВЫБОР ТОПОЛОГИИ${NC}"
echo -e "${YELLOW}=================================================${NC}"
...
echo -e "\n${YELLOW}Скрипт успешно завершен! Проверь пинги.${NC}"
