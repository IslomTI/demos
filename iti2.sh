...
# Особенности: CD-ROM APT Fix, RAID5 Polling Warning, Services Only
# ==============================================================================
...
prepare_apt() {
    log_info "Очистка APT от сломанных CD-ROM репозиториев..."
...
setup_hqsrv() {
    ...
    prepare_apt
    apt-get install -y -q mdadm nfs-kernel-server apache2 mariadb-server php php-mysql bind9 bind9-utils
    ...
    if [ "$DISK_COUNT" -eq 3 ]; then
        ...
        log_err "ВНИМАНИЕ! Найдено только $DISK_COUNT дисков из 3. Сборка RAID5 пропущена! Вы забыли добавить 3 диска в настройках VMware."
    fi
...
EOF
    ...
    log_info "Настройка BIND9 Master сервера..."
...
EOF
    ...
    log_succ "HQ-SRV: RAID5, NFS, Web App (port 8082) и DNS (Bind9) готовы!"
}

setup_brsrv() {
    log_info "Установка Samba AD, Docker и Ansible..."
    prepare_apt
    apt-get install -y -q docker.io docker-compose-plugin samba smbclient krb5-user winbind libpam-winbind libnss-winbind ansible sshpass
...
EOF
    ...
    log_succ "BR-SRV: Samba AD, Docker App (8082) и Ansible готовы!"
}

setup_hqcli() {
    log_info "Проверка DNS (ожидание BIND9 от HQ-SRV)..."
    for i in {1..30}; do
    ...
    prepare_apt
    apt-get install -y -q nfs-common autofs realmd sssd sssd-tools adcli krb5-user dnsutils
...
echo -e "${CYAN} RAID5, Samba AD DC, Docker Mock, Ansible, APT Clean${NC}"
echo -e "${YELLOW}======================================================${NC}"
echo ""
echo "Выберите роль машины:"
echo "  1) ISP     — Провайдер (Nginx Reverse Proxy + Basic Auth)"
echo "  2) HQ-RTR  — Роутер Центра (Не требует сервисов L7)"
echo "  3) HQ-SW   — Коммутатор Центра (Не требует сервисов L7)"
echo "  4) BR-RTR  — Роутер Филиала (Не требует сервисов L7)"
