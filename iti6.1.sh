...
# Особенности: Strict DHCP Polling, AD DNS Forwarding, br_netfilter Fix
# ==============================================================================

trap cleanup SIGINT SIGTERM

cleanup() {
    echo -e "\n\033[0;31m[ВНИМАНИЕ] Скрипт прерван пользователем. Снимаю блокировки...\033[0m"
...
EOF
    ...
    # КРИТИЧЕСКИЙ ФИКС: Загрузка модуля моста перед применением sysctl
...
EOF
    ...
    force_dns "127.0.0.1"
    log_succ "HQ-SRV: Базовая сеть, SSH и Master DNS развернуты!"
}
...
EOF
    ...
    log_info "Проверка связности DNS (ожидание ответа от HQ-SRV)..."
    for i in {1..15}; do
        if ping -c 1 -W 2 archive.ubuntu.com >/dev/null 2>&1 || ping -c 1 -W 2 8.8.8.8 >/dev/null 2>&1; then
            log_succ "DNS и маршрутизация функционируют."
            break
        fi
        sleep 3
    done
...
EOF
    ...
    log_succ "HQ-CLI: Нативный клиент настроен!"
}
...
echo -e "${CYAN} Production-Ready: Чистый Netplan и DORA Polling${NC}"
echo -e "${YELLOW}======================================================${NC}"
echo ""
