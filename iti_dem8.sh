#!/bin/bash
# ==============================================================================
# Master Script - V8 (Фикс Ansible, Портов, Sudo, Строгий Способ 1 - VLAN)
# ==============================================================================
...
# Для HQ-CLI оставляем стандартный порт 22, чтобы Ansible мог достучаться
create_sshuser_cli() {
    id -u sshuser &>/dev/null || useradd -m -s /bin/bash -u 1015 sshuser
    echo "sshuser:P@ssw0rd" | chpasswd
    usermod -aG sudo sshuser
...
setup_hqrtr() {
    ...
    read -p "Интерфейс ВНУТРЬ (Один кабель/Trunk): " INT_LAN
...
network:
  ...
  tunnels:
    gre1: {mode: gre, local: $HQ_ISP_IP, remote: $BR_ISP_IP, addresses: [$GRE_HQ/30]}
EOF
    apply_netplan
...
EOF
    ...
    echo -e "local stratum 5\nallow 192.168.0.0/16\nallow 10.0.0.0/8" >> /etc/chrony/chrony.conf
...
network:
  ...
  tunnels:
    gre1: {mode: gre, local: $BR_ISP_IP, remote: $HQ_ISP_IP, addresses: [$GRE_BR/30]}
EOF
    apply_netplan
...
EOF
    ...
    read -p "Введи 3 диска для RAID (или Enter для пропуска): " DISK1 DISK2 DISK3
    if [ -n "$DISK1" ]; then
...
setup_brsrv() {
    ...
    # Устанавливаем sshpass для работы Ansible
    apt-get install -y -q docker.io docker-compose samba smbclient winbind krb5-user chrony ansible sshpass
...
EOF
    ...
    # НАСТРОЙКА ANSIBLE (Отключение проверки ключей и задание портов)
    mkdir -p /etc/ansible
...
[routers]
$HQ_RTR_V10 ansible_user=net_admin ansible_port=22 ansible_password='P@\$\$w0rd'
$BR_RTR_LAN ansible_user=net_admin ansible_port=22 ansible_password='P@\$\$w0rd'

[servers]
$HQ_SRV_IP ansible_user=sshuser ansible_port=3015 ansible_password=P@ssw0rd

[clients]
hq-cli.$DOMAIN ansible_user=sshuser ansible_port=22 ansible_password=P@ssw0rd
EOF
...
clear
echo -e "${YELLOW}=================================================${NC}"
echo -e "${GREEN} ЭКЗАМЕН - ФИНАЛЬНАЯ ВЕРСИЯ (СПОСОБ 1 - VLAN)${NC}"
echo -e "${YELLOW}=================================================${NC}"
