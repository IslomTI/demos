...
# Настройка базовой инфраструктуры, VLAN, GRE, OSPF, DHCP, DNS, SSH
# ==============================================================================
...
# Принудительная очистка сломанных пакетов APT (защита от ошибки 100)
rm -rf /var/lib/apt/lists/*
rm -f /var/lib/dpkg/lock*
rm -f /var/cache/apt/archives/lock
dpkg --configure -a 2>/dev/null || true

# Временный DNS для скачивания пакетов
...
# Пользователь sshuser для Серверов (Порт 3015)
create_sshuser() {
...
# Пользователь net_admin для Роутеров
create_netadmin() {
...
EOF
    ...
    # Настройка Динамической Трансляции (PAT/NAT) для доступа в интернет
...
EOF
    ...
    # NAT на роутере не нужен, он на ISP. Но маскарадинг бывает полезен.
    iptables -t nat -A POSTROUTING -o $INT_ISP -j MASQUERADE
    mkdir -p /etc/iptables
    iptables-save > /etc/iptables/rules.v4

    # ИДЕАЛЬНЫЙ OSPF (Пункт 7: Только в туннеле)
...
EOF
    ...
    # ИДЕАЛЬНЫЙ DNS (Пункт 10: Зоны A и PTR)
...
EOF
    ...
    # Обратная зона для сети 192.168.10.x
...
EOF
    ...
    # Обратная зона для сети 192.168.20.x
...
clear
echo -e "${YELLOW}=================================================${NC}"
echo -e "${GREEN} ДЕМОЭКЗАМЕН - МОДУЛЬ 1 (Облегченная версия)${NC}"
echo -e "${YELLOW}=================================================${NC}"
