...
# Особенности: Udev Polling, Moodle L7 Fix, Samba TCP Polling, Safe Trap
# ==============================================================================
...
setup_hqsrv() {
    ...
    log_info "Активация модуля RemoteIP для прозрачных логов Moodle..."
    a2enmod remoteip
...
EOF
    ...
    log_info "Модификация php.ini: Увеличение max_input_vars для Moodle..."
    find /etc/php -name "php.ini" -exec sed -i 's/^;*max_input_vars.*/max_input_vars = 5000/' {} \;
    systemctl restart apache2

    log_info "Ищем 3 пустых диска для RAID0 (LVM-Safe метод)..."
    AVAILABLE_DISKS=""
    ...
    if [ "$DISK_COUNT" -eq 3 ]; then
        ...
        log_succ "Найдены чистые диски: $RAID_TARGETS"
        ...
        log_info "Форсирование обновления базы udev и ожидание генерации UUID..."
        udevadm trigger
        udevadm settle
        partprobe /dev/md0
        ...
        MD_UUID=""
        for i in {1..30}; do
            MD_UUID=$(blkid -p -s UUID -o value /dev/md0 || lsblk -no UUID /dev/md0)
            if [ -n "$MD_UUID" ]; then
                log_succ "Успешно получен UUID для RAID0: $MD_UUID (Попытка $i)"
                break
            fi
            log_info "UUID недоступен. Ожидание подсистемы udev..."
            sleep 1
        done
        
        if [ -n "$MD_UUID" ]; then
            if ! grep -q "$MD_UUID" /etc/fstab; then
                echo "UUID=$MD_UUID /raid0 ext4 defaults 0 0" >> /etc/fstab
            fi
            mkdir -p /raid0
            mount -a
        else
            log_err "ФАТАЛЬНО: Подсистема ядра не вернула UUID для /dev/md0. Сборка массива прервана."
            exit 1
        fi
    else
...
SQLEOF
    ...
    log_info "Скачивание ядра Moodle (v4.0.4)..."
    cd /tmp
    if ! wget -q --timeout=45 --tries=3 https://download.moodle.org/download.php/direct/stable404/moodle-latest-404.tgz; then
        log_err "Фатальный сбой скачивания Moodle! Установка LMS пропущена во избежание краха скрипта."
        return 1
    fi
    
    tar -xzf moodle-latest-404.tgz -C /var/www/html/
    
    mkdir -p /var/www/moodledata
    chown -R www-data:www-data /var/www/html/moodle /var/www/moodledata

    log_info "Настройка Apache DocumentRoot под директорию Moodle..."
    sed -i 's|DocumentRoot /var/www/html|DocumentRoot /var/www/html/moodle|g' /etc/apache2/sites-available/000-default.conf
...
setup_brsrv() {
    ...
    apt-get install -y -q docker.io docker-compose-plugin \
        samba smbclient krb5-user winbind libpam-winbind libnss-winbind acl attr iproute2

    echo "$BR_SRV_IP br-srv.$DOMAIN br-srv" >> /etc/hosts

    mkdir -p /home/sshuser/wiki
    cat > /home/sshuser/wiki/docker-compose.yml <<EOF
version: '3.8'
services:
  wiki:
...
EOF
    ...
    log_info "Инициализация домена Samba AD DC (СТРОГАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ ДО DOCKER)..."
...
EOF
    ...
    log_info "Ожидание полной инициализации DNS ядра Samba AD..."
    for i in {1..60}; do
        if ss -tln | grep -q ":53 "; then
            if samba-tool domain info "$DOMAIN_UPPER" >/dev/null 2>&1; then
                log_succ "Samba AD DC RPC и DNS интерфейсы активны!"
                break
            fi
        fi
        
        if [ "$i" -eq 60 ]; then
            log_err "Таймаут инициализации Samba AD DNS! Интеграция Docker может завершиться сбоем."
        fi
        sleep 2
    done
    ...
    log_info "Запуск контейнеров MediaWiki (AD DNS уже функционирует)..."
    docker compose up -d || { log_err "Docker Compose упал. Прерывание."; exit 1; }
    
    log_info "Ожидание запуска контейнера базы данных (State: running)..."
    for i in {1..60}; do
        STATE=$(docker inspect -f '{{.State.Running}}' wiki_db 2>/dev/null)
        if [ "$STATE" == "true" ]; then
            break
        fi
        sleep 2
    ...
    log_info "Ожидание инициализации схемы внутри MariaDB..."
    for i in {1..30}; do
        if docker exec wiki_db mysqladmin ping -u root -proot --silent 2>/dev/null; then
            if docker exec wiki_db mysql -u root -proot -e "USE mediawiki;" 2>/dev/null; then
                log_succ "MariaDB готова к приему подключений!"
                break
            fi
        fi
        sleep 3
    ...
    log_info "Ожидание готовности ядра MediaWiki..."
    for i in {1..30}; do
        if docker exec wiki_app stat /var/www/html/maintenance/install.php >/dev/null 2>&1; then
    ...
    log_succ "BR-SRV: Samba AD DC и MediaWiki (Persisted Volumes) успешно настроены!"
}
...
setup_hqcli() {
    ...
    log_info "Ожидание генерации DNS SRV-записей (через локальный резолвер BIND9)..."
    for i in {1..40}; do
        if dig +short -t SRV _ldap._tcp.$DOMAIN | grep -q "br-srv"; then
            log_succ "SRV-записи успешно разрешаются через локальный BIND9!"
            break
        fi
        sleep 3
    done
...
echo -e "${CYAN} Production-Ready: Clean Syntax, Udev Polling, Moodle L7 Fix${NC}"
echo -e "${YELLOW}======================================================${NC}"
echo ""
