#!/bin/bash
# ==============================================================================
# ДЕМОЭКЗАМЕН 2025 - МОДУЛЬ 1 (v11 - Исправленная версия)
# Исправлены: пароль net_admin, персистентность маршрутов ISP, префиксы сетей.
# ==============================================================================
...
set_hostname_and_time() {
    ...
    # ВНИМАНИЕ: Измените зону, если сдаете не в MSK!
...
setup_isp() {
    ...
    # ИСПРАВЛЕНО: Маршруты вынесены в Netplan для персистентности после ребута
...
network:
  ...
  ethernets:
    ...
    $INT_HQ: 
      ...
      routes: 
        - {to: 192.168.10.0/27, via: $HQ_ISP_IP}
        - {to: 192.168.20.0/27, via: $HQ_ISP_IP}
        - {to: 192.168.99.0/29, via: $HQ_ISP_IP}
    $INT_BR: 
      optional: true
      addresses: [$ISP_BR_IP/28]
      routes:
        - {to: 192.168.30.0/28, via: $BR_ISP_IP}
EOF
    apply_netplan
...
setup_hqsrv() {
    ...
    show_interfaces
    read -p "Интерфейс (Тот, что смотрит в Trunk HQ-RTR): " INT_LAN
...
EOF
    ...
    echo -e "${GREEN}[УСПЕХ] HQ-CLI настроен (Получает IP по DHCP, DNS зафиксирован)!${NC}"
}
