#!/bin/bash
# ==============================================================================
# Master Script - V5 (Исправлен DNS, FRR, Умная загрузка)
# ==============================================================================
...
# Принудительно даем временный DNS для скачивания пакетов
...
apply_netplan() {
    ...
    netplan apply
    sleep 3
    # Возвращаем управление DNS системной службе
    ...
    $INT_ISP:
      ...
      routes:
        - to: default
          via: $ISP_HQ_IP
      nameservers:
        addresses: [8.8.8.8, 1.1.1.1] # Исправление DNS
    $INT_LAN:
      optional: true
      dhcp4: false
  vlans:
...
clear
echo -e "${YELLOW}=================================================${NC}"
echo -e "${GREEN} ЭКЗАМЕН - ФИНАЛЬНАЯ ВЕРСИЯ (С ФИКСАМИ DNS)${NC}"
echo -e "${YELLOW}=================================================${NC}"
...
echo -e "\n${YELLOW}Скрипт успешно завершен!${NC}"
