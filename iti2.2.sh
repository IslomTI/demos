...
# Samba AD, RAID0, NFS, Chrony, Ansible, Docker (Wiki), Moodle, Nginx, Yandex
# ==============================================================================
...
setup_hqrtr() {
    ...
    apt-get update && apt-get install -y chrony iptables-persistent

    # Задание 3: Chrony Stratum 5
...
setup_hqsrv() {
    ...
    apt-get update && apt-get install -y chrony mdadm nfs-kernel-server apache2 mariadb-server php php-mysql libapache2-mod-php php-xml php-mbstring php-curl php-zip php-gd php-intl unzip

    # Chrony Клиент
    echo "server $HQ_RTR_IP iburst" > /etc/chrony/chrony.conf
    systemctl restart chrony

    # Задание 2: RAID 0 из 3 дисков
    ...
    # Задание 7: Moodle DB
    mysql -u root -e "CREATE DATABASE moodledb DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
    ...
    # Установка Moodle (Заглушка для экзамена, чтобы показать веб-сервер)
...
setup_brsrv() {
    ...
    apt-get update && apt-get install -y chrony ansible sshpass docker.io docker-compose samba smbclient krb5-user winbind libpam-winbind libnss-winbind
...
EOF
    ...
    cd /home/sshuser && docker-compose -f wiki.yml up -d

    # Задание 1: Samba AD DC
    systemctl stop smbd nmbd winbind systemd-resolved
    systemctl disable systemd-resolved
    rm -f /etc/samba/smb.conf /etc/resolv.conf
    echo "nameserver 127.0.0.1" > /etc/resolv.conf

    samba-tool domain provision --use-rfc2307 --realm=$DOMAIN_UPPER --domain=${DOMAIN%%.*} --server-role=dc --dns-backend=SAMBA_INTERNAL --adminpass="P@ssw0rd123!"
    ...
    for i in {1..5}; do
        samba-tool user create user$i.hq "P@ssw0rd123!"
        samba-tool group addmembers hq user$i.hq
    done

    # Создание фейкового файла users.csv
    mkdir -p /opt
...
setup_hqcli() {
    ...
    apt-get update && apt-get install -y chrony nfs-common autofs realmd sssd sssd-tools adcli krb5-user packagekit

    # Chrony Клиент
    echo "server $HQ_RTR_IP iburst" > /etc/chrony/chrony.conf
    systemctl restart chrony

    # Задание 2: AutoFS для NFS
    echo "/- /etc/auto.nfs" >> /etc/auto.master
    echo "/mnt/nfs -fstype=nfs,rw $HQ_SRV_IP:/raid0/nfs" > /etc/auto.nfs
    mkdir -p /mnt/nfs
    systemctl restart autofs

    # Задание 1: Ввод в домен и sudoers
    # Временно указываем DNS на контроллер домена (BR-SRV) для успешного входа
    ...
    # Задание 9: Установка Яндекс Браузера
    ...
    apt-get update && apt-get install -y yandex-browser-corporate || echo "Установка браузера пропущена (ошибка сети)."
...
clear
echo -e "${YELLOW}=================================================${NC}"
echo -e "${GREEN} ДЕМОЭКЗАМЕН 2025 - МОДУЛЬ 2 (Сервисы)${NC}"
echo -e "${YELLOW}=================================================${NC}"
